...
def choose_stock_finance():
    dataframe = get_fundamentals(
        query(
            fundamentals.equity_valuation_indicator.pe_ratio_nobtest, fundamentals.equity_valuation_indicator.pb_ratio,
            fundamentals.equity_valuation_indicator.market_cap_2, fundamentals.equity_valuation_indicator.ps_ratio_ttm,
            fundamentals.equity_valuation_indicator.dividend_yield, fundamentals.equity_valuation_indicator.peg_ratio
        ).filter(
            fundamentals.equity_valuation_indicator.pe_ratio_nobtest > 0
        ).filter(
            fundamentals.equity_valuation_indicator.pe_ratio_nobtest < 40
        ).filter(
            fundamentals.equity_valuation_indicator.pb_ratio > 0
        ).filter(
            fundamentals.equity_valuation_indicator.pb_ratio < 10
        ).filter(
            fundamentals.equity_valuation_indicator.market_cap_2 > 1544000
        ).filter(
            fundamentals.equity_valuation_indicator.market_cap_2 < 8045602184879
        ).filter(
            fundamentals.equity_valuation_indicator.ps_ratio_ttm < 100
        ).filter(
            fundamentals.equity_valuation_indicator.dividend_yield < 10
        ).filter(
            fundamentals.equity_valuation_indicator.peg_ratio > 1
        ).filter(
            fundamentals.equity_valuation_indicator.peg_ratio < 100000
        )
    )
